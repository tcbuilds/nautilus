...
Source of truth is the personal Claude Code config at `~/.claude/`:
...
1. Reads the source from `~/.claude/skills/<name>/SKILL.md` or `~/.claude/agents/<name>.md`.
...
- After meaningfully upgrading the source SKILL.md or agent definition in `~/.claude/`.
- After publishing the playbook publicly and discovering a sanitization gap that the regex pass should have caught — patch the sanitizer in `~/.claude/skills/nautilus-sync/scripts/sanitize.py`, then re-sync everything affected.

## When not to invoke

- For skills or agents that are inherently personal (pull private transcripts, hardcoded to one user's services, depend on family or revenue context). These don't get sanitized — they get excluded.
...
The sanitizer covers the recurring patterns observed across personal skills and agents:
...
The regex table is in `~/.claude/skills/nautilus-sync/scripts/sanitize.py`. Add new patterns there as new leakage vectors emerge.

## Lifecycle in one sentence

Mature the source in `~/.claude/`, sync downstream into nautilus when the entry is ready, review the diff, commit and push.
